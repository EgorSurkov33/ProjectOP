...
def modz():
    ...
    global wisans
    global counter
    md = int(input(' 1 - Тренировочный, 2 - Контрольный: '))
    if md == 1:
        wisans = 1
        tren()
    if md == 2:
...
def tren():
    ...
    r2.destroy()
    button.destroy()
    title = Label(frameZag, text='Тренировочный режим',font=40)
    title.pack()
    title2 =Label(frameMain, text='Выберите тип задач:\n 1 - Передача звуковых файлов,\n 2 - Хранение звуковых файлов,\n 3 - Передача изображений,\n 4 - Передача текстовых файлов,\n 5 - Хранение изображений,\n 6 - Определение размера записанного файла,\n 7 - Определение времени передачи данных: ',font=40)
    title2.pack()
    lbox=Listbox(frameMain,width=40, height=10)
    lbox.pack()
    for i in ('1 - Передача звуковых файлов',' 2 - Хранение звуковых файлов',' 3 - Передача изображений', '4 - Передача текстовых файлов', '5 - Хранение изображений', '6 - Определение размера записанного файла',' 7 - Определение времени передачи данных'):
        lbox.insert(0,i)
    typ = 1
    kol = 3
    for i in range(kol):
...
def contr():
    ...
    r2.destroy()
    button.destroy()
    title = Label(frameZag, text='Контрольный режим',font=40)
...
window = Tk()
window.title("Тренажер 7 задания ЕГЭ")
window.geometry('700x550')  
window.resizable(width=False, height=False)
canvas = Canvas(window, height=700, width=550)
canvas.pack()
frameMain = Frame(window, bg='grey')
frameMain.place( relwidth=1, relheight=1)
frameZag = Frame(frameMain, bg='LightSteelBlue1', height=150)
frameZag.pack(fill=tk.X)
title = Label(frameZag, text='Добро пожаловать в тренажер 7 задания ЕГЭ! Выберите один из режимов работы:',font=40)
title.pack()
r_var = IntVar()
r_var.set(0)
r1 =Radiobutton(frameMain,text=' 1 - Тренировочный',variable=r_var, value=1)
r2 =Radiobutton(frameMain,text=' 2 - Контрольный',variable=r_var,value=2)

r1.pack(anchor=W)
r2.pack(anchor=W)
button = Button(frameMain,text="Изменить",command=change)
button.pack()
